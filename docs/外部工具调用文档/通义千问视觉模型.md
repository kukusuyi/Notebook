...
| --- | --- | --- | --- | --- | --- |
| `Qwen3.6`系列、`Qwen3.5`系列 | 支持  | 支持  | `qwen3.6-plus`、`qwen3.6-flash`、`qwen3.5-plus`、`qwen3.5-flash` > `qwen3.6-flash`、`qwen3.5-flash`仅支持显式缓存 | 非思考模式支持 | 33种；分别为中文、日语、韩语、印尼语、越南语、泰语、英语、法语、德语、俄语、葡萄牙语、西班牙语、意大利语、瑞典语、丹麦语、捷克语、挪威语、荷兰语、芬兰语、土耳其语、波兰语、斯瓦希里语、罗马尼亚语、塞尔维亚语、希腊语、哈萨克语、乌兹别克语、宿务语、阿拉伯语、乌尔都语、波斯语、印地语 / 天城语、希伯来语。 |
...
识别图像中的文字、公式或抽取票据、证件、表单中的信息，支持格式化输出文本；支持的语言可参见[模型特性对比](#a47aeb6443ysh)。
...
-   `qwen3.6`、`qwen3.5`、`qwen3-vl-plus`、`qwen3-vl-flash`系列模型属于混合思考模型，模型可以在思考后回复，也可直接回复；通过`enable_thinking`参数控制是否开启思考模式：
    
    -   `true`：开启思考模式。`qwen3.6`、`qwen3.5`系列模型默认为`true`。
...
-   **图像大小：**
    ...
    -   以公网 URL 传入时：Qwen3.6系列、Qwen3.5系列单个图像不超过 `20MB`，其他模型单个图像不超过`10MB`
...
-   **以视频文件传入时：**
    ...
    -   **视频大小：**
        ...
        -   以公网 URL 传入时：
            ...
            -   `qwen3.6`系列、`qwen3.5` 系列、`Qwen3-VL`系列、`qwen-vl-max`（包含`qwen-vl-max-latest`、`qwen-vl-max-2025-04-08` 之后的所有版本）：不超过 2GB；
    ...
    -   **计算图像与视频的Token：**可通过以下代码计算图像或视频的 Token 消耗。估算结果仅供参考，实际用量以 API 响应为准。
        ...
        **计算图像与视频的Token**
        ...
        -   `token_pixels`：每视觉`Token`对应的像素值，不同模型情况不同：
            ...
            -   `qwen3.6系列`、`qwen3.5系列`、`Qwen3-VL`、`qwen-vl-max`、`qwen-vl-max-latest`、`qwen-vl-max-2025-08-13`、`qwen-vl-plus`、`qwen-vl-plus-latest`、`qwen-vl-plus-2025-08-15``、qwen-vl-plus-2025-07-10`**：**每个`Token`对应 `32x32`像素
