<template>
 <div class="layout-root" :class="{'immersive':!topLevel}">
  <aside class="sidebar">
   <RouterLink class="brand" to="/dashboard"><strong>题迹 <small>Questrace</small></strong></RouterLink>
   <nav class="nav-list" aria-label="主要导航"><RouterLink v-for="item in desktopNavItems" :key="item.to" :to="item.to" class="nav-item" :class="{active:isActive(item.to)}" :aria-label="item.label"><el-icon><component :is="item.icon"/></el-icon><span>{{item.label}}</span></RouterLink></nav>
  ...
  <UpdateNotice/><div class="layout-main">
...
import UpdateNotice from '@/components/UpdateNotice/index.vue'
...
const desktopNavItems=[...navItems.slice(0,2),{label:'复习',short:'复习',to:'/reviews',icon:Collection},...navItems.slice(2)]
