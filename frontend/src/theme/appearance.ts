...
export type AppearancePreferences = {version:1; preset:Preset; mode:'system'|'light'|'dark'; accentSeed:string|null}
export const appearanceKey = 'notebook:appearance:v1'
export const defaults:AppearancePreferences = {version:1,preset:'blue',mode:'system',accentSeed:null}
export function normalizeAppearance(value: unknown): AppearancePreferences {
  ...
  return {version:1,preset:p.preset && p.preset in themes?p.preset:'blue',mode:['system','light','dark'].includes(p.mode||'')?p.mode!:'system',accentSeed:/^#[\da-f]{6}$/i.test(p.accentSeed||'')?p.accentSeed!:null}
}
let stored:unknown
