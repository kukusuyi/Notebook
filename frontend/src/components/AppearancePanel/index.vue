<template>
  <section class="paper-card appearance-panel">
    ...
    <div class="background-controls"><label class="background-upload">选择背景图片<input type="file" accept="image/jpeg,image/png,image/webp" @change="chooseBackground"/></label><el-button v-if="appearance.background" @click="removeBackground">移除背景</el-button></div>
    <p class="meta-text">图片只保存在此浏览器，不会上传到电脑服务。支持 10 MB 以内的 JPG、PNG、WebP。</p>
...
import { ref } from 'vue'
import {validateBackground} from '@/theme/background'
import { appearance, defaults, saveBackground, setAppearance, themes, type AppearancePreferences } from '@/theme/appearance'
const backgroundError=ref('')
async function removeBackground(){try{await saveBackground(null)}catch{backgroundError.value='背景移除失败，请重试'}}
async function restore(){await removeBackground();setAppearance(defaults)}
async function chooseBackground(e:Event){
 const input=e.target as HTMLInputElement,file=input.files?.[0];if(!file)return
 backgroundError.value=''
 try{validateBackground(file)}catch(e){backgroundError.value=(e as Error).message;input.value='';return}
 try{
