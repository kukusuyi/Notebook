...
func (s *LocalRuntime) updates(w http.ResponseWriter, r *http.Request) {
	...
	info, err := service.Releases.LatestFor(r.Context(), platform, arch, buildinfo.Version)
	if err != nil {
	...
	dto.WriteSuccess(w, map[string]any{"version": info.Version, "current_version": buildinfo.Version, "description": info.Notes, "release_url": info.URL, "download_url": info.Asset(platform, arch), "platform": platform, "arch": arch, "source": info.Source})
}
